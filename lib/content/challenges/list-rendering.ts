...
test('preserves per-item input state across deletion (stable keys)', () => {
  ...
  // Type a note into the SECOND item's uncontrolled input.
  ...
  // Delete the FIRST item ("Buy groceries").
  const deleteButtons = screen.getAllByRole('button', { name: /delete/i });
  fireEvent.click(deleteButtons[0]);

  // "Walk the dog" is the only item left and must keep its note.
  // With key={index} React reuses the first row's empty input → note lost.
